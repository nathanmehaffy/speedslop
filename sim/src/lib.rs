...
mod tests {
    ...
    fn assert_core_invariants(sim: &Simulation) {
        ...
        for index in 0..pop {
            ...
            assert!(
                (0.0..=1.0).contains(&speed_norm),
                "speed_norm out of range: {speed_norm}"
            );

            assert!(
                (0.0..=sim.world_size).contains(&sim.pos_x[index]),
                "pos_x out of range"
            );
            assert!(
                (0.0..=sim.world_size).contains(&sim.pos_y[index]),
                "pos_y out of range"
            );
            assert!(
            ...
            assert!(
                ...
                "direction not unit length: {dir_len}"
            );
        }
    }
    ...
    fn sanitize_population_clamps_to_bounds() {
        ...
        assert_eq!(
            sanitize_population(MAX_POPULATION as u32 + 1),
            MAX_POPULATION
        );
    ...
    fn rng_next_signed_f32_in_range() {
        ...
        for _ in 0..100_000 {
            ...
            assert!(
                (-1.0..1.0).contains(&v),
                "next_signed_f32 out of range: {v}"
            );
        }
    }
    ...
    fn fast_tanh_matches_std_tanh_within_tolerance() {
        for i in -400..=400 {
            ...
            let std = x.tanh();
            assert!((fast - std).abs() < 0.025, "x={x}: fast={fast}, std={std}");
            assert!(
                (-1.0..=1.0).contains(&fast),
                "fast_tanh out of range at x={x}"
            );
        }
    }
    ...
    fn randomize_genome_bounds_and_isolation() {
        ...
        for i in 0..sim.genomes.len() {
            if (start..start + GENOME_LEN).contains(&i) {
                assert!(
                    (-0.75..=0.75).contains(&sim.genomes[i]),
                    "gene out of range"
                );
            } else {
    ...
    fn mutate_changes_stay_within_magnitude_and_limit() {
        ...
        for g in &genome {
            ...
            assert!(
                g.abs() <= MUTATION_MAGNITUDE + 1e-6,
                "change exceeded magnitude: {g}"
            );
        }
    }
    ...
    fn evaluate_network_isolated_per_agent() {
        ...
        assert!(
            outputs.iter().any(|o| *o != 0.0),
            "nonzero genome must produce output"
        );
    }
        ...
        for ray in 0..RAY_COUNT {
            ...
            for k in 0..RAY_INPUTS {
                assert_eq!(
                    inputs[base + k],
                    0.0,
                    "ray {ray} component {k} should be empty"
                );
            }
        }
        let self_base = RAY_COUNT * RAY_INPUTS;
    ...
    fn vision_detects_agent_directly_ahead() {
        ...
        assert!(
            inputs[base + 5].abs() < 1e-5,
            "aligned agent has zero relative heading"
        );
    }
    ...
    fn vision_lateral_gate_includes_and_excludes() {
        ...
        assert_eq!(
            inside[RAY_COUNT / 2 * RAY_INPUTS],
            1.0,
            "just inside lateral width"
        );
    ...
    fn vision_buckets_into_correct_side_ray() {
        ...
        assert_eq!(
            inputs[RAY_COUNT / 2 * RAY_INPUTS],
            0.0,
            "center ray should stay empty"
        );
    }
    ...
    fn vision_includes_ninety_degree_rays() {
        ...
        assert_eq!(
            inputs[last_ray], 1.0,
            "+90 degree ray should see the left agent"
        );
    ...
    fn breeding_is_sticky_to_current_partner() {
        ...
        assert!(
            (sim.mate_timer[0] - 2.0 * dt).abs() < 1e-6,
            "timer must keep growing"
        );
    }
    ...
    fn generation_reports_current_max_alive() {
        ...
        assert_eq!(
            sim.generation(),
            0,
            "generation must reflect current max, not peak"
        );
    }
